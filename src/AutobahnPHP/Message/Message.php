...
abstract class Message {
    ...
    public function isValidForConnection(Wamp2Connection $conn) {
        ...
        foreach ($this->getValidConnectionStates() as $connState) {
            if ($connState == Wamp2Connection::STATE_ALL) return true;
            if ($conn->getConnectionState() == $connState) return true;
        }
    ...
    static public function createMessageFromRaw($rawMsg) {
        ...
        switch ($json[0]) {
            ...
            case Message::MSG_PUBLISH:
                $args = null;
                if (count($json) >= 5) $args = $json[4];

                $argsKw = null;
                if (count($json) >= 6) $argsKw = $json[5];
